...
import { Baby, Search, X, ChevronDown, ChevronUp, AlertTriangle, Phone, Lightbulb } from "lucide-react";
import clsx from "clsx";
import { PageTransition } from "@/components/PageTransition";
import type { GuideSection, GuideCategory } from "@/types";
import feedingData from "../../../../data/feeding_guide.json";

const ALL_SECTIONS: GuideSection[] = feedingData.sections as GuideSection[];
...
const CATEGORY_OPTIONS: { value: GuideCategory; label: string; emoji: string; color: string }[] = [
  ...
  { value: "routine", label: "Routine", emoji: "🔄", color: "bg-sage-100 text-sage-700 border-sage-200 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-800" },
...
const BADGE_STYLES: Record<GuideCategory, string> = {
  ...
  routine: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300",
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-pink-100 dark:bg-pink-900/30 rounded-xl">
            <Baby size={22} className="text-pink-600 dark:text-pink-400" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-neutral-900 dark:text-neutral-100">Feeding Guide</h1>
            <p className="text-xs text-neutral-500 dark:text-neutral-400">{ALL_SECTIONS.length} topics · breastfeeding, bottle & troubleshooting</p>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search feeding topics…"
            className="w-full pl-8 pr-8 py-2.5 text-sm bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-pink-300 dark:focus:ring-pink-700"
          />
          {query && (
